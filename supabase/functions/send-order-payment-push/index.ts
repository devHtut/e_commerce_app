...
Deno.serve(async (req) => {
  ...
  let requestBody: { orderId?: string };
  try {
  ...
  if (order.customer_id !== user.id) {
    return json(
      { success: false, message: 'This order does not belong to you.' },
      403,
    );
  }
  ...
  if (brandIds.size === 0) {
    return json(
      { success: false, message: 'No vendor brands found for this order.' },
      404,
    );
  }
  ...
  if (vendorIds.length === 0) {
    return json(
      { success: false, message: 'No vendor owners found for this order.' },
      404,
    );
  }
  ...
  const { data: profile } = await admin
    ...
    .eq('id', user.id)
    .maybeSingle();
  const customerName = profile?.full_name?.toString().trim() || 'Customer';
  const readableId = order.readable_id?.toString() || orderId;
  const itemSummary = buildItemSummary(itemNames, itemCount);
  const title = 'New paid order';
  const body =
    `${customerName} paid for order #${readableId}: ${itemSummary}. Total: ${
      formatKyat(Number(order.total_price ?? 0))
  ...
  if (tokens.length === 0) {
    return json(
      { success: false, message: 'No active vendor push tokens found.' },
      404,
    );
  }
    ...
    return json(
      {
        success: sentCount > 0,
        message: sentCount > 0
          ? 'Order payment push sent.'
          : 'Unable to send order payment push.',
        sentCount,
